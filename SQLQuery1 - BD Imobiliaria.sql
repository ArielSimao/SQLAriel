...
Postado por Clayton Klen às 06:45 Nenhum comentário: 
Enviar por e-mail
BlogThis!
...
hora_ida time,
hora_retorno time)
Postado por Clayton Klen às 05:00 Nenhum comentário: 
Enviar por e-mail
BlogThis!
Compartilhar no Twitter
Compartilhar no Facebook
...
Postado por Clayton Klen às 12:03 Nenhum comentário: 
Enviar por e-mail
BlogThis!
Compartilhar no Twitter
Compartilhar no Facebook
...
Postado por Clayton Klen às 12:00 Nenhum comentário: 
Enviar por e-mail
BlogThis!
Compartilhar no Twitter
Compartilhar no Facebook
...
Empresa - Banco de Dados - Guia Prático de Aprendizagem - Exercício Completos
...
--3. Selecionar Nome e Cargo dos Clientes e Funcionaros que são do Reino Unido.
SELECT Nome, Cargo
FROM Clientes
UNION
SELECT Nome, Cargo
...
--19 Selecionar todos Pedidos feitos por Nome de Funcionarios começados com a letra "A"
SELECT F.Nome, P.*
...
--46 Fazer uma View com todos Produtos com Preco abaixo da Média.
...
PRINT 'Produto já cadastrado para esse pedido'
ELSE
BEGIN
...
END;
END;
EXEC Insere_Detalhes 10868, 199, 1, 1, 1 --Codigo do Produto não Cadastrado!
EXEC Insere_Detalhes 100, 11, 1, 1, 1 --Numero do Pedido não Cadastrado!
EXEC Insere_Detalhes 10867, 53, 1, 1, 1 --Produto já cadastrado para esse pedido
...
FROM Produtos
WHERE CodProd = @CodProd))
PRINT 'Não há produto cadastrado com esse código para ser excluído!';
ELSE
BEGIN
...
END;
EXEC Exclui_Produto 444 --Não há produto cadastrado com esse código para ser excluído!
...
FROM Produtos
WHERE CodProd = @CodProd))
PRINT 'Não há produto cadastrado com esse código para ser alterado!';
ELSE
BEGIN
UPDATE Produtos
SET Descr=@DescrProd
...
END;
EXEC Altera_Produto 444, 'chocolate' --Não há produto cadastrado com esse código para ser alterado!
...
FROM Pedidos
WHERE NumPed = @numPed))
PRINT 'Não há pedido cadastrado com esse número!';
ELSE
BEGIN
...
WHERE Cidade=@cidade
END;
EXEC Clientes_Cidade 'Jundiaí' --Não há clientes cadastrado para essa cidade
...
AND P.Descr = 'Guaraná Fantástica'
...
WHERE CodProd IN (SELECT CodProd
FROM Produtos
WHERE Descr = 'Guaraná Fantástica')))
...
INSERT INTO Fornecedores
VALUES ('Editora Erica', 'João', 'Editor', 'Rua de São Paulo','São Paulo', '', 'Brasil')
...
Uma consulta mal projetada pode acarretar em lentidão ao servidor ou até mesmo fazer com que o
servidor permaneça fora do ar por um período, impossibilitando o usuário de utilizar algum serviço do
sistema ou site, gerando prejuízo a organização.
Otimizar uma consulta SQL nem sempre é uma tarefa fácil e simples, precisa um pouco de conhecimento
para desenvolver filtros que obtenham resultados que não custe ao servidor muitos recursos de hardware.
Muitos dos exemplos que irei demonstrar foram resultados de pesquisas e testes executados no próprio
Query Analyser do MS SQL Server 2000.
Uma forma de prejudicar bastante o desempenho ao acessar a base de dados, é a criação de TRIGGERS.
Esta instrução é acionada sempre que uma determinada tabela sofre alguma alteração (INSERT, UPDATE, DELETE).
Portanto, ao invés de criar uma ‘TRIGGER‘ prefira criar uma PROCEDURE que será acionada sempre que uma tarefa for feita.
...
/*
Ao fazer algum filtro em uma tabela, evite informar todos os campos de uma tabela em uma consulta, a menos que isso for realmente necessário.
 Muitas das vezes não será utilizado todos os campos.
 A consulta acima pode ser executada rapidamente se a tabela da base de dados contiver poucos registros,
 mas para adquirir uma melhor performance da consulta é recomendável filtrar apenas os campos que serão utilizados,
 o exemplo abaixo demonstra um cenário parecido:
 */
...
*/
  ...
  Clientes
/*
Porém não há a necessidade de utilização desta consulta, porque ao executá-la, será contado um registro de cada vez.
Para isto existe as tabelas ‘sysobjects‘ e ‘sysindexes‘.
Com estas duas tabelas é possível obter muitas informações de diversos todos objetos existentes na base de dados.
*/
...
Reparem que foi utilizado uma nova instrução, o INNER JOIN.
Esta instrução serve para fazer a junção de duas ou mais tabelas,
lembrando que isto significa que DEVE possuir algum registro na tabela secundária,
ou seja na tabela que está recebendo o INNER JOIN, sendo assim,
é necessário fazer a comparação de campos em comum que as duas possuem.
Certifique-se que o nome da tabela escrito na cláusula WHERE está correto,
caso contrário não será retornado nenhum registro.
Esta consulta é outro exemplo que poderá trazer problemas de performance, porque ao utilizá-la,
a consulta toda pode acessar qualquer uma das informações (campos) existentes nas tabelas.
 Uma outra forma de consultar a quantidade de registros de uma tabela é a seguinte:
 */
...
/*
Ao fazer um filtro como o acima, pode-se obter resultados muito eficazes em relação ao desempenho.
A instrução EXISTS é uma instrução extremamente rápida porque utiliza pouco recurso de hardware.
É como se retornasse uma variável booleana.
Outra instrução que consome um maior processamento é a instrução IN,
que é utilizada quando é necessário fazer o filtro de um vetor de dados.
...
/*
São inúmeras as tarefas que se pode fazer para melhorar a performance de uma consulta.
Estas são apenas algumas operações que devem ser feitas caso necessita de resultados mais rápidos em consultas SQL.
*/